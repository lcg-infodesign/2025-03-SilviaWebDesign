...
function draw() {
  ...
  for (let i=0; i<data.getRowCount(); i++) {
    ...
    const nomeVulcano = riga.get("Volcano Name") || "Unknown";
    ...
    dammiTrasparenzaVulcano(status, trasparenzaVulcanoAttivo, trasparenzaVulcanoDormiente, altriStatiVulcanici);
    ...
    if (distanza < raggio){
      //text(paese, x, y);
      vulcanoSelezionato = {paese, nomeVulcano, tipo, altezza, x, y};
    }

     // se il vulcano è hovered evidenzia bordo
    if (vulcanoSelezionato && vulcanoSelezionato.nomeVulcano === nomeVulcano) {
      stroke(200);
      strokeWeight(2);
      ellipse(x, y, raggio*2);
      noStroke();
